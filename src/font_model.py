...
def variables_conv(label_size, device):
...
def model_conv(variables, data, dropout_prob, device):
    with tf.device(device):
...
class FontModel:
    def __init__(self,
        ...
        with tf.device(device):
            ...
            variables = variables_conv(label_size, device)

            logits = model_conv(variables, self.input_data, self.dropout_prob, device)
